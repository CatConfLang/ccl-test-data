...
// crlf_normalize_to_lf - behavior:crlf-normalize-to-lf function:parse variant:proposed-behavior (level 1)
func TestCrlfNormalizeToLf(t *testing.T) {
	t.Skip("Test does not match run-only filter: [basic essential-parsing empty redundant quotes realistic line-endings]")
}

// crlf_preserve_literal - behavior:crlf-preserve-literal function:parse variant:reference-compliant (level 1)
func TestCrlfPreserveLiteral(t *testing.T) {
	t.Skip("Test does not match run-only filter: [basic essential-parsing empty redundant quotes realistic line-endings]")
}
